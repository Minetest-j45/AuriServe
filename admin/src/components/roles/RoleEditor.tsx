...
export default function RoleEditor(props: Props) {
	...
	const handleIdentifierChange = (e: any) => {
		let role = Object.assign(JSON.parse(JSON.stringify(props.role)), { identifier: e.target.value });
		props.setRole(role);
	};
		...
		return (
			<Preact.Fragment>
				<label class='RoleEditor-AbilitySwitch'>
					...
					<input type='checkbox' checked={props.role.abilities.filter(f => f === RoleAbility[ability]).length > 0}
	...
	return (
		<div class='RoleEditor'>
			<label class='RoleEditor-Field'>
				<span>Role Name</span>
				<input type='text' style={props.role.color && {color: props.role.color}} onChange={handleIdentifierChange}
					onInput={handleIdentifierChange} class='RoleEditor-IdentifierInput' value={props.role.identifier} />
			</label>
